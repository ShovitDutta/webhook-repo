...
def process_push_event(payload):
    ...
    timestamp = payload["repository"]["pushed_at"] if "pushed_at" in payload["repository"] else datetime.now(timezone.utc).isoformat()
    event_data = {
        ...
        "from_branch": None,
        "to_branch": branch,
        "timestamp": datetime.fromtimestamp(timestamp, timezone.utc) if isinstance(timestamp, int) else datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%SZ")
    }
